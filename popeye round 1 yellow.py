...
##        pass
    ...
    for i in contours_factor:
        ...
        if cv2.contourArea(i)>50:#CHANGE AREA
...
def error_detection(theta,angle1, angle2):
    ...
    arduino(theta,angle1-10,angle2)
...
##    mask = mask0+mask1
    ...
    yellowmask=cv2.erode(yellowmask,kernel,iterations=15)
...
##    upper_blue=np.array([140,255,255])
    ...
    #yellowmask=cv2.erode(yellowmask,kernel,iterations=1)#remove
    #yellowmask=cv2.dilate(yellowmask,kernel,iterations=1)#remove
    cv2.imshow('bluemask',yellowmask)#change to mask
    
    print("in mask")

    img2,contours,hierarchy=cv2.findContours(yellowmask,cv2.RETR_TREE,cv2.CHAIN_APPROX_SIMPLE)#change to mask
...
while True:
    ...
    if len(targets)>0:#CHANGE TO TWO
        cv2.imshow('i',img)
